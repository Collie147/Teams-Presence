...
# Date 04.11.2020
#Modified by Colin Russell-Conway
# Date 05.05.2021
...
try:
    ...
    import sys
    import urllib.parse
    import time, getopt, re, subprocess
    ...
    for t in threading.enumerate():
        if t is main_thread:
            ...
            try:
                ...
                print("exception in thread join")
    switchOff()
    exit(0)
def setBackString(textString, foreground, backString):
    global back
    global text
    global fore
    text = textString
    ...
    items = []
    if backString.lower() in colours:
        back = colours[backString]
    else:
        back = ImageColor.getrgb(backString.lower())
    while getattr(t, "do_run", True):
        #print ('running backstring thread', 'text:', text, 'fore:', fore, 'back:', back, end="\r")
        
        item = {'text': text, 'fore':fore}
        items.append(item)
        image = render(items)
        scroll(image)
...
def switchBlue(text) :
    global mode, blinkThread
    red = 0
    green = 0
    blue = 250
    modeText = "switchBlue-" + text
    if mode != modeText:
        mode = modeText
        blinkThread.do_run = False
        blinkThread.join()
        #blinkThread = threading.Thread(target=setColor, args=(red, green, blue, '', ''))
        blinkThread = threading.Thread(target=setBackString, args=(text, "white", "blue"))
...
def switchRed(text) :
    ...
    green = 0
    blue = 0
    modeText = "switchRed-" + text
    if mode != modeText:
        mode = modeText
        blinkThread.do_run = False
        blinkThread.join()
        #blinkThread = threading.Thread(target=setColor, args=(red, green, blue, '', ''))
        blinkThread = threading.Thread(target=setBackString, args=(text, "white", "red"))
        blinkThread.do_run = True
        blinkThread.start()

def switchGreen(text) :
    global mode, blinkThread
    red = 0
    green = 250
    blue = 0
    modeText = "switchGreen-" + text
    if mode != modeText:
        mode = modeText
        blinkThread.do_run = False
        blinkThread.join()
        #blinkThread = threading.Thread(target=setColor, args=(red, green, blue, '', ''))
        blinkThread = threading.Thread(target=setBackString, args=(text, "white", "green"))
        blinkThread.do_run = True
        blinkThread.start()
...
def switchPink(text) :
    ...
    green = 108
    blue = 180
    modeText = "switchPink-" + text
    if mode != modeText:
        mode = modeText
        blinkThread.do_run = False
        blinkThread.join()
        #blinkThread = threading.Thread(target=setColor, args=(red, green, blue, '', ''))
        blinkThread = threading.Thread(target=setBackString, args=(text, "white", "pink"))
        blinkThread.do_run = True
        blinkThread.start()

def switchYellow(text) :
    global mode, blinkThread
    red = 255
    green = 255
    blue = 0
    modeText = "switchYellow-" + text
    if mode != modeText:
        mode = modeText
        blinkThread.do_run = False
        blinkThread.join()
        #blinkThread = threading.Thread(target=setColor, args=(red, green, blue, '', ''))
        blinkThread = threading.Thread(target=setBackString, args=(text, "white", "yellow"))
        blinkThread.do_run = True
        blinkThread.start()
